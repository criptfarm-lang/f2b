...
SUPPLIER_HINT_MAP = {
    ...
    "ultrafish": "ultrafish",
    "ультрафиш": "ultrafish",
    "ультра фиш": "ultrafish",
    "ультра-фиш": "ultrafish",
    "ultra fish": "ultrafish",
    "ultra-fish": "ultrafish",
    # Остров-Фиш
    "остров": "ostrov-fish",
    "ostrov": "ostrov-fish",
    # Русская Рыбная Компания (АО РРК)
    "русская рыбная компания": "русская-рыбная-компания",
    "русская рыбная": "русская-рыбная-компания",
    "ао русская": "русская-рыбная-компания",
    "ррк": "русская-рыбная-компания",
    # Асаги-Фиш
    "асаги фиш": "асаги-фиш",
    "асаги-фиш": "асаги-фиш",
    "асаги": "асаги-фиш",
    "asagi fish": "асаги-фиш",
    "asagi": "асаги-фиш",
    # Мегаполис-Продукт (мясо/птица)
    "мегаполис продукт": "мегаполис-продукт",
    "мегаполис-продукт": "мегаполис-продукт",
    "мегаполис": "мегаполис-продукт",
    # Вкусман
    "вкусман": "вкусман",
    # Tamaki (японская кухня, рис)
    "tamaki": "tamaki",
    "тамаки": "tamaki",
    # Фуд-Трейд
    "фуд трейд": "фуд-трейд",
    "фуд-трейд": "фуд-трейд",
    "food trade": "фуд-трейд",
    # Дифиш
    "дифиш": "дифиш",
    "difish": "дифиш",
...
def _build_system_prompt() -> str:
    return f"""Ты парсер прайс-листа поставщика для F2B - комплексного снабжения ресторанов и HoReCa.

ИЗВЛЕКАЙ ВСЕ товарные позиции прайса БЕЗ ИСКЛЮЧЕНИЯ. Один лот = одна позиция одной
ступени цены. Любая категория товара - продукты, ингредиенты, полуфабрикаты, готовая
продукция, упаковка, тара, инвентарь - всё, что встречается в прайсе.

Ассортимент F2B растёт каждый месяц. Сегодня прилетел Якимал (специи и японская кухня),
завтра может быть что угодно. Не оценивай «подходит ли это F2B» - просто извлекай все
позиции которые ты видишь в таблице.

ENUM ниже частично заполнен рыбными значениями (исторический скоуп F2B). Когда позиция
вписывается в один из ENUM - используй точное значение. Когда не вписывается - ставь
species='прочее', subspecies=реальное название из прайса, processing='unspecified',
state='unspecified' или ближайший подходящий. Это не делает позицию хуже - она тоже
извлекается.
...
ПОДСКАЗКИ для маппинга в ENUM (когда подходит):
...
ПОДСКАЗКИ когда позиция НЕ вписывается в ENUM (любая категория, кроме рыбы):
- species="прочее"
- subspecies = реальное название позиции из прайса (точно как написано)
- processing="unspecified"
- state: сухой/сыпучий → "сушёный"; замороженный → "глубокая-заморозка" или "IQF";
  охлаждённый → "охл"; комнатной температуры / не применимо → "unspecified"
- weight_class = вес одной штуки или упаковки из прайса (например "110 гр", "1 кг",
  "100 шт"). Если веса нет в прайсе → "unspecified"
- product_form="сырьё" по умолчанию (рыбный enum, но это безопасный fallback)
- price_rub_kg = цена ЗА ШТУКУ или ЗА ЕДИНИЦУ (НЕ за упаковку из N штук). Если в
  прайсе колонки «Цена/руб.» (за штуку) и «Цена упаковки/руб.» (за упаковку) → бери
  ПЕРВУЮ. Это критично.
- confidence_self="confirmed" по умолчанию. Позиция извлекается в основную выдачу
  наравне с любой другой.

Структура товарных таблиц обычно одинаковая, независимо от категории. Часто колонки:
Арт. | Наименование | Срок годности/Технология | Вес/гр | Цена/руб (за штуку) |
В упаковке/шт | Цена упаковки. Бери: subspecies=Наименование, weight_class=Вес,
price_rub_kg=Цена/руб (за штуку), notes=Артикул+Технология+Срок+Упаковка.

confidence_self='archived' (мусор, не товар): доставка, реквизиты, телефоны, ИНН,
заголовки разделов, шапки таблиц, сумма заказа, банковские реквизиты, метки
"Акции"/"Скидки" без позиции. Эти строки не извлекаем как лоты вообще.
...
async def parse_photo_message(image_path: str, caption: str = "", original_filename: str = "") -> dict:
    ...
    return await _call_claude([
        ...
        {"type": "text", "text": (
            f"Фото прайс-листа поставщика F2B.\n"
            f"Caption: {caption or '(нет)'}\n"
            f"Оригинальное имя файла: {original_filename or '(нет)'}\n"
            f"Извлеки ВСЕ товарные позиции которые видишь на фото. Не оценивай категорию.\n"
            f"supplier_hint бери из caption / имени файла / логотипа или шапки на фото."
        )},
    ])


def _flatten_xls_to_text(xls_path: str, max_chars_per_sheet: int = 30000,
                          total_max_chars: int = 90000) -> str:
    """Открывает XLS/XLSX и возвращает плоский text-дамп всех листов/строк/ячеек.

    Sonnet не умеет читать XLS как native document, поэтому делаем flatten:
    ```
    === Sheet: 'Прайс-лист' ===
    R1: ФАРВАТЕР | ИНН 7813266850 | Санкт-Петербург | ...
    R6: # | МОРЕПРОДУКТЫ | ПРОИЗВОДИТЕЛЬ | СПб | Мск
    R7: 1 | Капуста МОРСКАЯ 16+ | Китай | 505,00 | 509,00
    ```

    Fix (29.05 вечер): лимит per-sheet (не глобальный), чтобы 3-й sheet 'Прайс-лист'
    в Якимал (после первых двух 'РЛ' и 'FAQ') не обрезался досрочно. Каждый sheet
    получает до 30k chars, общий лимит 90k chars (~22k tokens, безопасно для tier-1).
    """
    ext = xls_path.lower().rsplit(".", 1)[-1]
    parts = []
    total_len = 0

    def _emit(line: str, sheet_len: int) -> tuple[bool, int]:
        """Возвращает (can_continue, new_sheet_len)."""
        nonlocal total_len
        clipped = line[:500]
        parts.append(clipped)
        total_len += len(clipped) + 1
        sheet_len += len(clipped) + 1
        if total_len > total_max_chars:
            parts.append(f"\n... (truncated at total {total_max_chars} chars)\n")
            return False, sheet_len
        if sheet_len > max_chars_per_sheet:
            parts.append(f"\n... (sheet truncated at {max_chars_per_sheet} chars, continuing next sheet)\n")
            return False, sheet_len
        return True, sheet_len
    ...
    if ext == "xlsx":
        ...
        for sheet in wb.worksheets:
            ...
            for r_idx, row in enumerate(sheet.iter_rows(values_only=True), start=1):
                ...
                cont, sheet_len = _emit(line, sheet_len)
                if not cont:
                    break
            if total_len > total_max_chars:
                break
    ...
    content = [
        {
            "type": "text",
            "text": (
                f"XLS/XLSX-прайс поставщика F2B. Плоский дамп всех листов формата:\n"
                f"«Rн: cell1 | cell2 | cell3 | ...» где Rн - номер строки.\n"
                f"Шапка таблицы (R1-R3 или похожие) даёт контекст колонок. Если есть колонки\n"
                f"«СПб» и «Мск» — бери цену из колонки «Мск» (F2B работает в Москве).\n"
                f"Игнорируй реквизиты, шапки-заголовки, телефоны - бери только товарные строки.\n\n"
                f"Caption от пересылающего: {caption or '(нет caption)'}\n"
                f"Оригинальное имя файла: {original_filename or '(нет)'}\n"
                f"   ВАЖНО: имя файла часто содержит бренд поставщика - \n"
                f"   «ЯКИМАЛ Прайс МАЙ 2026.xlsx» → supplier_hint=\"Якимал\". \n"
                f"   Используй имя как ПЕРВЫЙ источник supplier_hint когда caption пустой.\n\n"
                f"supplier_hint - источники в порядке приоритета:\n"
                f"  1) Оригинальное имя файла (бренд в начале имени)\n"
                f"  2) Шапка таблицы / логотип в первых строках\n"
                f"  3) Первое чёткое название бренда / реквизиты юрлица в шапке\n"
                f"НЕ возвращай: «прайс», «склад», «прайс-лист», «Мониторинг», «ФИШ ТУ БИЗНЕС».\n\n"
                f"Извлекай ВСЕ товарные строки таблицы из ВСЕХ листов. Не оценивай категорию.\n\n"
                f"Дамп таблицы:\n{flat_text}"
            ),
        },
    ]
    ...
    content = [
        ...
        },
        {
            ...
            "text": (
                ...
                f"supplier_hint - источники в порядке приоритета:\n"
                f"  1) Имя файла (бренд в начале имени)\n"
                f"  2) Шапка PDF (логотип/название компании в верхних строках)\n"
                f"  3) Реквизиты (ИНН/название юрлица если явно поставщик)\n"
                f"НЕ возвращай: «Мониторинг», «Эф», «ФИШ ТУ БИЗНЕС», «прайс», «прайс-лист».\n"
                f"Если бренда совсем нет — supplier_hint=null.\n\n"
                f"Извлекай ВСЕ товарные строки таблицы. Не оценивай категорию.\n"
                f"Шапку, реквизиты, секции-заголовки CAPS — игнорируй (контекст, не лот)."
            ),
        },
    ]
