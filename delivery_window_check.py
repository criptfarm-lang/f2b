...
def format_message(rows: list[dict]) -> str:
    """Текст пинга менеджеру. Без Markdown: в названиях клиентов живут кавычки и звёздочки."""
    head = ("⏰ Не проставлено время приёмки\n\n"
            "Логистика не угадывает пустую границу — окно разворачивается на весь день "
            "(с 06:00), водитель приезжает к открытию, а точка может принимать позже.\n")
    body = "\n".join(_order_line(r) for r in rows[:_MAX_ROWS])
    tail = ""
    if len(rows) > _MAX_ROWS:
        tail = f"\n\n…и ещё {len(rows) - _MAX_ROWS} заказ(ов)."
    return (f"{head}\n{body}{tail}\n\n"
            "Проставьте в заказе «Окно доставки с (время)» и «до (время)» — "
            "часы, в которые точка реально принимает.")
