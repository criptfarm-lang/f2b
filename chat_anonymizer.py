...
_MANAGER_FULL_NAMES = [
    ...
    "Кормилицын Антон", "Антон Кормилицын",
]

# ── Регэкспы сущностей ────────────────────────────────────────────────────────
# Все юникодные разновидности дефиса и тире. Без них телефон из подписи менеджера
# «8‑968‑547‑93‑49» (неразрывный дефис U+2011) проходил мимо маски, а find_leaks
# при этом возвращал «чисто» — обнаружено 23.09.2026 на переписке сделки 44721919.
# Неразрывные пробелы отдельно перечислять не нужно: \s их уже ловит.
_HY = "\\-\u2010-\u2015\u2212\ufe63\uff0d"
...
_RE_PHONE = re.compile(
    rf"(?:\+7|\b8|\b7)[\s{_HY}(]*\d{{3}}[\s{_HY})]*\d{{3}}[\s{_HY}]*\d{{2}}[\s{_HY}]*\d{{2}}\b"
)
