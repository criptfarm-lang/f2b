...
# Ларгус (26695) в таблицах вместимости пока НЕТ — собственник даёт цифру после
# первых загрузок. Пока её нет, _volume_note по этой машине печатает объём без
# предела и без предупреждения о перегрузе (см. ветку `if not cap_boxes`).
