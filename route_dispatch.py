...
# 8267564735 — Белякова, 1296942948 — Боева А. (вышла 08.09.2026, забирает
# логистику на себя; добавлена 09.09.2026 после /start). Петровский В. снят
# с логистики 07.09.2026 (ушёл учётчиком партий в цех) — его id убран из
# дефолта. Список — env LOGIST_CHAT_IDS (через запятую); добавить логиста =
# дописать id.
...
_CAP_BOXES = {26210: 190, 26209: 165, 26695: 28}  # К459 Porter / В970 KIA — «руками»
# (0 паллетов); Ларгус — 28 коробов (собственник 2026-08-04).
...
_CAP_BY_PALLETS = {
    ...
    # Ларгуса (26695) здесь СОЗНАТЕЛЬНО нет: паллеты в неё не грузим никогда
    # (собственник 2026-08-04), поэтому её вместимость всегда 28 коробов ручной
    # загрузки — _cap_boxes_for фолбэчит на _CAP_BOXES, что и требуется.
}
