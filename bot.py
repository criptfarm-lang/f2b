...
EMPLOYEES = {
    ...
    "Антон Кормилицын": [
        "антон", "антона", "антону", "антоном",
        "кормилицын", "кормилицына", "кормилицыну",
    ],
}
...
async def _build_report_data() -> dict:
    ...
    MONTHLY_PLANS = {
        ...
        # АКБ на сентябрь сверен с админкой «Планы ОП» (op_plans) 07.09.2026 — там его
        # ведёт собственник, и хардкод отставал: Карина 44→60, Ирина 5→20, Денис 15→20,
        # Инесса 28→29. Выручка сходилась до рубля и не менялась. У Инессы план полный,
        # с Фугу: op_plans[скляр] + op_plans[фугу] = 9+12 млн и АКБ 25+4.
        # Отгрузки, новые клиенты и привл. товары в админке не хранятся вовсе —
        # остаются на прежних значениях, их источник только здесь.
        "2026-09": {
            "Инесса Скляр":     {"shipments": 170, "revenue": 21_000_000, "clients": 29, "new_clients": 5, "attracted": 1_000_000},
            "Карина Баласанян": {"shipments": 170, "revenue": 6_000_000,  "clients": 60, "new_clients": 5, "attracted": 1_100_000},
            "Елена Мерзлякова": {"shipments": 55,  "revenue": 6_700_000,  "clients": 20, "new_clients": 5, "attracted": 300_000},
            "Ирина Дьяченко":   {"shipments": 10,  "revenue": 3_500_000,  "clients": 20, "new_clients": 1, "attracted": 12_500},
            "Денис Коликов":    {"shipments": 30,  "revenue": 10_000_000, "clients": 20, "new_clients": 5, "attracted": 50_000},
            # Первый месяц Антона в отчёте. Выручка и АКБ — из op_plans (собственник
            # задал их в админке «Планы ОП»). Отгрузки / новые / привл. товары он не
            # ставил: держим 0, а не выдуманное число. Метрики с планом 0 не входят
            # в средний процент (см. avgPct в report_html.py).
            "Антон Кормилицын": {"shipments": 0,   "revenue": 300_000,    "clients": 10, "new_clients": 0, "attracted": 0},
        },
        "2026-10": {
...
def main():
    ...
    # до него не дойдут) и рабочие инструменты развозки: это не «ответы
    # ассистента», без них встаёт логистика.
    #
    # 28.08.2026: в первый же день после блока логист не смог собрать развозку —
    # перехват съел весь логистический контур, а не только ответы ассистента.
    # Возвращены целиком: сборка и подтверждение маршрутов, реестр/листы склада,
    # ход развоза, превью статусов отгрузки. Права внутри самих команд остались
    # (route_dispatch._allowed, route_registry._allowed, delivery_statuses) —
    # логист, водители и владелец; постороннему команда ответит отказом.
    # Кириллические алиасы обязательны: у логиста в ходу именно они.
    ALLOWED_COMMANDS = {
        "/start",
        # водитель
        "/reis", "/рейс", "/shipmentlist", "/лист",
        # логист: маршруты на подтверждение, реестр, листы склада, ход, статусы
        "/routes", "/маршруты",
        "/registry", "/реестр",
        "/sklad", "/склад",
        "/progress", "/ход",
    ...
    # Страница /route/<uid>/<дата> читает готовый снимок и отдаётся мгновенно; без
    # прогрева её строил своим кликом сам водитель и упирался в троттл МС (при 429 —
    # sleep 30 c), телефон рвал соединение. План 2026-08-07-живой-реестр-водителя.
    # ────────────────────────────────────────────────────────────────────
    import route_web as _route_web

    async def _route_web_warm(context):
        try:
            from scheduler import MSK as _MSK_TZ
            if not (5 <= datetime.now(_MSK_TZ).hour < 22):
                return   # ночью маршрутов нет, МС дёргать незачем
            await _route_web.warm_all()
        except Exception as e:
    ...
    # ────────────────────────────────────────────────────────────────────
    # Приход по заказу поставщику не заведён на остатки — 20:00 МСК ежедневно
    # в группу «F2B СКЛАД». Только сырьё, пищевые добавки, упаковка и
    # привлечённые товары. План: 2026-09-07-алерт-склад-приход-не-заведён-
    # на-остатки.
    # ────────────────────────────────────────────────────────────────────
    from arrival_check import poll_job as _arrival_check_poll

    async def _arrival_check_job(context):
        stats = await _arrival_check_poll(app)
        if stats:
            logger.info(f"arrival_check job: {stats}")

    app.job_queue.run_daily(
        _arrival_check_job,
        time=_dt_time(hour=17, minute=0, tzinfo=_tz_for_job.utc),
    )
    logger.info("arrival_check: джоба зарегистрирована на 20:00 МСК")
