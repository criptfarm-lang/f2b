...
- Перечисление товаров в одном поручении НЕ дроби: «забрать тунец и скумбрию 1 шт» —
  это ОДИН пункт «Забери тунец и скумбрию, 1 шт». Количество, указанное один раз, не
  размножай по позициям и не додумывай, к какой из них оно относится.
- Разные действия — разные пункты: «забрать тару и отдать акт» это два пункта.
...
async def _tasks_via_llm(raw: str, money_required: bool) -> dict:
    ...
    for t in (data.get("tasks") or []):
        ...
        if not t:
            continue
        # Денежный пункт рознице режем сами, а не надеемся на модель: в тесте 14.08.2026
        # она оставляла «Забери наличными 33 300 ₽» рядом с авто-шагом «Забери деньги» —
        # водитель видел деньги дважды и не понимал, сколько раз их брать.
        if money_required and _MONEY_RE.search(t):
            continue
