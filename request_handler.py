...
угорь, тунец, скумбрия, сельдь.
ВАЖНО — народные/коммерческие названия видов рыбы: «стрелозубый/стрелозубое/
стрелозубовое» — это subspecies палтуса (species=палтус, subspecies=стрелозубый);
«синекорый» — тоже подвид палтуса; «чёрный палтус»/«гренландский» — палтус.
«рыба-меч» = species=рыба-меч; «нототения» = species=нототения; «клыкач/чилийский
сибас/морской волк» = species=клыкач; «эсколар/масляная Trichiurus» = species=эсколар.
Если попадается неизвестный вид рыбы — НЕ выдумывай новое значение species,
сначала попробуй найти ближайшее из списка выше, или верни species=прочее
и положи оригинал в subspecies для матчера.
...
КАТЕГОРИЧЕСКИ: НИКОГДА не используй species="другое". Для всего, что не вписано
в перечисленные значения species — используй species="прочее", а оригинальное
название положи в subspecies (или раздели на species + subspecies, если это
подвид известной рыбы — например species=палтус subspecies=стрелозубый).
...
subspecies: для КРЕВЕТКИ обязательно (это первичный признак, без него
закупщик не отличит лот за 450 ₽ от лота за 5000 ₽). Известные подвиды:
ваннамей, тигровая, северная, гребенчатая, карабинера, аргентинская
(лангустин/red shrimp), глубоководная, атлантическая, азовская, чёрная,
коктейльная. Записывай ТОЧНО как в исходном тексте (Карабинера, не
Aristaeomorpha; ваннамей, не Litopenaeus vannamei). Если в тексте есть
размерный класс (16/20, 21/25, 21+, 250+) — это weight_class, НЕ
subspecies. Для ИКРЫ: лососёвая/кетовая/щучья/форелевая. Регион —
отдельное поле.
