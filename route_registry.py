...
26695 (Ларгус Е 898 СР 797, добавлена 2026-08-04, GPS-трекер с 17.09.2026 — время
визита vt у её точек в раскладке есть, как у остальных; без vt порядок идёт по окну tf).
...
def stop_day(s):
    ...
    у водителя). У точек вне раскладки vt нет → работает tf.
    """
...
# Лимит точек в ОДНОЙ ссылке Яндекс.Карт. Проверено в браузере 2026-08-24: ссылки на 16,
# 20, 21, 25 и 30 точек открываются целиком, хвост НЕ режется (прежние «~10» из веб-справки
# не подтвердились). На 20 точках панель адресов заполняется штатно; свыше 20 Яндекс
# показывает плашку «На маршруте может быть только 20 точек» — маршрут строит, но это его
# объявленный предел, и как поведёт себя приложение Карт на телефоне водителя — не проверено.
# Поэтому держим 20. Больше — дробим на части с перекрытием.
YA_MAX_WAYPOINTS = 20
