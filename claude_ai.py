...
async def detect_task_completion(text: str, open_tasks: list) -> list:
    ...
    prompt = f"""Сообщение из рабочего чата: "{text}"

Открытые задачи:
{tasks_str}

Определи: какие из этих задач закрывает данное сообщение?
Отвечай ТОЛЬКО JSON массивом ID задач. Если ни одна — пустой массив.
Пример: [3, 7] или []
