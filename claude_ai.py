...
ВАЖНО: get_overdue = просроченные ЗАДАЧИ (дедлайн по задаче прошёл)
get_overdue_debt = просроченные ДОЛГИ/ОПЛАТЫ (клиент не заплатил вовремя)
Если в запросе упоминается компания/клиент/контрагент рядом со словом "просрочка" — это ВСЕГДА get_overdue_debt
