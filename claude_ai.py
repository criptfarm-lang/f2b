...
- get_overdue: показать просроченные ЗАДАЧИ (только если спрашивают о задачах/дедлайнах, НЕ использовать если речь о деньгах/долгах/оплате/клиентах/компаниях)
...
- get_overdue_debt: просроченная дебиторская задолженность — все или по группе/менеджеру/клиенту (параметры: tag или query, опционально)
⚠️ КРИТИЧНО: 
  "просрочка у [компания]" → get_overdue_debt
  "просроченный долг" → get_overdue_debt
  "просроченная задолженность" → get_overdue_debt
  "просроченные задачи" / "что просрочено" (без упоминания компании/денег) → get_overdue
