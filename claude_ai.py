...
Менеджеры по продажам (всего 4): Баласанян Карина, Мерзлякова Елена, Скляр Инесса, Леонтьев Алексей.
...
Примеры:
  ...
  "ПДЗ Инесса кратко" → get_overdue_debt (tag=скляр)
  "ПДЗ Карина" → get_overdue_debt (tag=баласанян)
  "ПДЗ Алексей" → get_overdue_debt (tag=леонтьев)
  "ПДЗ Елена" → get_overdue_debt (tag=мерзлякова)
  "просрочка у Джи Эф" → get_overdue_debt (query=Джи Эф)
  "просроченные задачи" → get_overdue
Слово "кратко" означает краткий формат вывода, не меняет тип действия.
...
- search_mentions: найти упоминания товара в переписках менеджеров через Wazzup (параметры: product — товар или список товаров через запятую, days — за сколько дней, по умолчанию 7, manager — имя менеджера если нужен конкретный)
- op_report: отчёт ОП — план vs факт по менеджерам (пришли/покажи/сформируй отчёт ОП, отчёт отдела продаж)
- aging: показать стареющих клиентов — тех кто не отгружался 40+ дней (стареющие клиенты, клиенты без отгрузок, давно не покупали, не отгружались более месяца/недели/дней)
- send_message_to_client: отправить сообщение конкретному клиенту через Wazzup (параметры: client — название клиента, message — текст сообщения; если текст не указан — сформировать напоминание об оплате)
- manager_activity: отчёт по активности менеджера за период — количество звонков и сообщений (параметры: manager — имя менеджера или "все", days — за сколько дней, по умолчанию 7)
- generate_contract: создать договор поставки для покупателя (параметры: buyer — название компании покупателя; остальные данные берутся из МойСклад или запрашиваются у менеджера)
- reconciliation: создать акт сверки с контрагентом (параметры: buyer — название компании, date_from — начало периода YYYY-MM-DD, date_to — конец периода YYYY-MM-DD; если не указан — текущий год)
...
Запрос: "пдз печи"
Ответ: {{"action": "get_overdue_debt", "params": {{"tag": "печи"}}, "confidence": 0.99}}

Запрос: "пдз рестораны"
Ответ: {{"action": "get_overdue_debt", "params": {{"tag": "хорека"}}, "confidence": 0.99}}

Запрос: "пдз по сегменту кафе"
Ответ: {{"action": "get_overdue_debt", "params": {{"tag": "кафе"}}, "confidence": 0.97}}

Запрос: "просрочка печи"
Ответ: {{"action": "get_overdue_debt", "params": {{"tag": "печи"}}, "confidence": 0.99}}
...
Запрос: "отчёт ОП"
Ответ: {{"action": "op_report", "params": {{}}, "confidence": 0.99}}

Запрос: "пришли отчет ОП"
Ответ: {{"action": "op_report", "params": {{}}, "confidence": 0.99}}

Запрос: "покажи отчет по продажам"
Ответ: {{"action": "op_report", "params": {{}}, "confidence": 0.97}}

Запрос: "отчёт отдела продаж"
Ответ: {{"action": "op_report", "params": {{}}, "confidence": 0.97}}

Запрос: "стареющие клиенты"
Ответ: {{"action": "aging", "params": {{}}, "confidence": 0.99}}Запрос: "клиенты которые не отгружались более месяца"
Ответ: {{"action": "aging", "params": {{}}, "confidence": 0.99}}

Запрос: "кто давно не покупал"
Ответ: {{"action": "aging", "params": {{}}, "confidence": 0.97}}

Запрос: "клиенты без отгрузок"
Ответ: {{"action": "aging", "params": {{}}, "confidence": 0.97}}
...
Запрос: "сформируй сверку ИТФИШ"
Ответ: {{"action": "reconciliation", "params": {{"buyer": "ИТФИШ"}}, "confidence": 0.99}}

Запрос: "акт сверки Стрелочка"
Ответ: {{"action": "reconciliation", "params": {{"buyer": "Стрелочка"}}, "confidence": 0.99}}

Запрос: "сделай акт сверки с Атмосфера за 2026"
Ответ: {{"action": "reconciliation", "params": {{"buyer": "Атмосфера", "date_from": "2026-01-01", "date_to": "2026-12-31"}}, "confidence": 0.99}}

Запрос: "акт сверки ИТФИШ с 01.01.2026 по 31.03.2026"
Ответ: {{"action": "reconciliation", "params": {{"buyer": "ИТФИШ", "date_from": "2026-01-01", "date_to": "2026-03-31"}}, "confidence": 0.99}}

Запрос: "сверка взаиморасчётов Биг Мама"
Ответ: {{"action": "reconciliation", "params": {{"buyer": "Биг Мама"}}, "confidence": 0.98}}
...
Запрос: "создай договор с Атмосфера"
Ответ: {{"action": "generate_contract", "params": {{"buyer": "Атмосфера"}}, "confidence": 0.99}}
...
- "мотивацию сделал" → закрывает задачу "разработать мотивацию"
- "отправил" / "готово" / "сделал" → закрывает задачу автора если она одна и смысл совпадает
- Опечатки и искажения слов не мешают распознаванию: "мотивауию" = "мотивацию", "отаправил" = "отправил"
...
НЕ закрывать если:
- это новое поручение ("Саша, сделай X", "Задача карине...", "Задача Алексею...")
- сообщение начинается со слова "Задача" — это постановка, не выполнение
- вопрос ("как дела с задачей?")
- обсуждение без конкретного результата
- исполнитель задачи — другой человек, не автор сообщения
- автор сообщения — руководитель (Виктор), а задача назначена на менеджера
