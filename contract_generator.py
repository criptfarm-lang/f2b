...
def generate_contract_pdf(data: dict) -> bytes:
    ...
    # Согласование «именуемый/именуемое» по орг.-правовой форме покупателя:
    # юрлицо (Общество/АО/ООО/ПАО/НКО и т.п.) — средний род «именуемое»,
    # ИП/физлицо — мужской род «именуемый».
    _bn_low = buyer_short.lower()
    _is_legal_entity = any(m in _bn_low for m in (
        "обществ", "акционерн", "«ао", "ао ", "оао", "пао", "ооо",
        "нко", "ассоциац", "фонд", "учреждени", "предприяти", "кооператив",
        "товариществ", "партнёрств", "партнерств",
    ))
    buyer_named = "именуемое" if _is_legal_entity else "именуемый"
    ...
    intro = (
        ...
        f"{buyer_short}, {buyer_named} в дальнейшем «Покупатель», "
            ...
            if rep_raw:
                ...
                if pos_words:
                    ...
                    POS_MAP = {
                        ...
                        "заместителя исполнительного директора": "Заместитель исполнительного директора",
                        "заместителя генерального директора": "Заместитель генерального директора",
                        "заместителя директора": "Заместитель директора",
